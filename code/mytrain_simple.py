...
import dataset_flmg as dataset
import model
...
flags.DEFINE_string('train_list', 'data/datalists/hmdb51_split1_train.txt', 'train list')
...
# Saving directory
flags.DEFINE_string('log_path', 'my_logs/coviar', 'save log dir')
flags.DEFINE_string('save_path', 'my_model/coviar', 'save model directotry')

flags.DEFINE_string('continue_training','my_model/coviar/all_net.chkp-1000', 'continue_step')
...
flags.DEFINE_float('tot_lr', 2e-6, 'learning rate')
...
def main(_):
    ...
    with tf.name_scope('classiciation_loss'):
        ...
        total_loss = combine_loss + i_class_loss + mv_class_loss + r_class_loss
    ...
    for step in range(FLAGS.max_steps):
        ...
        summary, _, pred, loss1, loss2, loss3, loss4 = sess.run([merged, train_opt, pred_class, mv_class_loss, i_class_loss, r_class_loss, combine_loss],
        ...
        if (step) % 10 == 0 :
            ...
            print('Step %d: %.3f sec' % (step, duration), 'mv_loss:', loss1,  'i_loss:', loss2,  'r_loss:', loss3, 'fusion_loss:', loss4)
            print('GT:', label)
            print('Pred:', pred)
