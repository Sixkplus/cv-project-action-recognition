        ...
        for seg_idx in range(0, nSegments):
            ...
            if img is None:
                ...
                if representation_idx == 1:
                    ...
                    # Testing: no need to obatin flow
...
def _parse_function_v2(filename, label, nSegments):
    ...
    for representation_idx in range (0, 3):
        ...
        for seg_idx in range(0, nSegments):
            ...
            if img is None:
                ...
                if representation_idx == 1:
                    ...
                    curr_img = load(filename.decode(), gop_index, gop_pos, 0, True)
                    prev_img = load(filename.decode(), gop_index, gop_pos-1, 0, True)
                    curr_img = cv2.cvtColor(curr_img, cv2.COLOR_BGR2GRAY)
                    prev_img = cv2.cvtColor(prev_img, cv2.COLOR_BGR2GRAY)
                    flow = compute_TVL1(prev_img, curr_img, bound=20)
                    #flow = (np.minimum(np.maximum(flow, 0), 255)).astype(np.uint8)
                    flow = np.append(flow, np.zeros_like(flow[...,0,None]), axis=-1)
                    flows.append(flow)
                    #print('Flow OK')
                elif representation_idx == 2:
                    img += 128
        ...
        if representation_idx == 0:
            ...
            np_frames = (np_frames - 0.5)
            #flow
            np_flows = np.array(flows).astype(np.float32) / 255.0
            np_flows = np_flows[:,16:240, 52:276, :].astype(np.float32)
        np_frames = np_frames [:,16:240, 52:276, :].astype(np.float32)
        reps_np.append(np_frames)

    return reps_np[0], reps_np[1], reps_np[2], np_flows, label
...
def buildTestDataset(test_list, data_path, nSegments, batch_size = 16, num_threads=2, buffer=30):
    ...
    dataset = tf.contrib.data.Dataset.from_tensor_slices((test_path_list, test_label_list)).repeat()

    dataset = dataset.map(lambda filename, label:
                          tuple( tf.py_func( _parse_function, [filename, label, nSegments],
                          [tf.float32, tf.float32, tf.float32, tf.int32])), output_buffer_size=buffer)
...
def buildTrainDataset_v2(train_list, data_path, nSegments, batch_size = 16, augment = True, shuffle = True, num_threads=2, buffer=30):
    ...
    dataset = tf.contrib.data.Dataset.from_tensor_slices((train_path_list, train_label_list)).repeat()
